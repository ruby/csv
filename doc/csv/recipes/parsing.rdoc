...
For strict compliance, use row separator "\r\n",
...
For data with non-compliant row separators, use option <tt>:row_sep</tt>.
...
you need not specify option <tt>:col_sep</tt> for compliant data:
  source = "foo,1\nbar,1\nbaz,2\n"
  CSV.parse(source) # => [["foo", "1"], ["bar", "1"], ["baz", "2"]]

===== Recipe: Handle Non-Compliant Column Separator

For data with non-compliant column separators, use option <tt>:col_sep</tt>.
...
you need not specify option <tt>:quote_char</tt> for compliant data:
  source = "\"foo\",\"1\"\n\"bar\",\"1\"\n\"baz\",\"2\"\n"
  CSV.parse(source) # => [["foo", "1"], ["bar", "1"], ["baz", "2"]]

===== Recipe: Handle Non-Compliant Quote Character

For data with non-compliant quote characters, use option <tt>:quote_char</tt>.
...
Use option <tt>:liberal_parsing</tt> to specify that \CSV should
