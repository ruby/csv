...
\Class method CSV::parse can read an \IO stream all at once:
  ...
  File.open(path, headers: true) do |file|
    CSV.parse(file)
  end # => [["Name", "Value"], ["foo", "0"], ["bar", "1"], ["baz", "2"]]
...
This example uses method CSV#<< to append the rows
that are to be generated:
  output_string = CSV.generate('', headers: ['Name', 'Value']) do |csv|
    csv << csv.headers
    csv << ['Foo', 0]
    csv << ['Bar', 1]
    csv << ['Baz', 2]
...
that are to be generated:
  path = 't.csv'
  CSV.open(path, 'w', headers: ['Name', 'Value']) do |csv|
    csv << csv.headers
    csv << ['Foo', 0]
    csv << ['Bar', 1]
    csv << ['Baz', 2]
...
\\Classs method CSV::new can generate \CSV data to an \IO stream:
  path = 't.csv'
  File.open(path, 'w') do |file|
    csv = CSV.new(file, headers: ['Name', 'Value'])
    csv << csv.headers
    csv << ['Foo', 0]
    csv << ['Bar', 1]
    csv << ['Baz', 2]
