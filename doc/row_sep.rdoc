...
The row separator: a \String or the \Symbol <tt>:auto</tt> (see below).
...
The String will be transcoded into the data's Encoding before parsing.
...
Using <tt>|</tt> (pipe):
  row_sep = '|'
  txt = CSV.generate(row_sep: row_sep) do |csv|
    csv << [:foo, 0]
    csv << [:bar, 1]
    csv << [:baz, 2]
  end
  txt # => "foo,0|bar,1|baz,2|"
  csv = CSV.parse(txt, row_sep: row_sep)
  csv # => [["foo", "0"], ["bar", "1"], ["baz", "2"]]
...
---
When +row_sep+ is the \Symbol +:auto+, invokes auto-discovery of the row separator.
